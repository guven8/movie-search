...
import { MovieSearchDTO, MovieService, MovieSearchResults, Movie } from '../services/movies';

const actionCreator = actionCreatorFactory();

export const searchMoviesAsync = actionCreator.async<MovieSearchDTO, MovieSearchResults>('GET_MOVIE_BY_SEARCH');
export function searchMovies(dto: MovieSearchDTO) {
  return async (dispatch) => {
    ...
    if (result.data.Response === 'True') {
      return dispatch(searchMoviesAsync.done({ params: dto, result: result.data }));
    }
...
export const getMovieByIdAsync = actionCreator.async<string, Movie>('GET_MOVIE_BY_ID');
export function getMovieById(imdbID: string) {
  return async (dispatch) => {
    ...
    if (result.data.Response === 'True') {
      return dispatch(getMovieByIdAsync.done({ params: imdbID, result: result.data }));
    }
